...
void create_stationery(Node *&head, int &many_product)
{
    ...
    new_node->product_name = ValidString("Masukkan nama produk: ", false);
...
void update_stationery(Node *head, int many_product)
{
    ...
    if (index > 0 && index <= many_product)
    {
        ...
        temp->product_name = ValidString("Masukkan nama produk baru: ", false);
...
void shell_sort(Node *head)
{
    ...
    while (gap > 0)
    {
        for (int i = 0; i < gap; i++)
        {
            ...
            for (Node *temp2 = temp1; temp2 != nullptr; temp2 = temp2->next)
            {
                Node *temp3 = temp2;
                while (temp3 != nullptr && temp3->next != nullptr && temp3->next->price > temp3->price)
                {
                    swap(temp3->price, temp3->next->price);
                    swap(temp3->product_name, temp3->next->product_name);
                    swap(temp3->merk, temp3->next->merk);
                    swap(temp3->stock, temp3->next->stock);
                    temp3 = temp3->next;
                }
            }
        }
